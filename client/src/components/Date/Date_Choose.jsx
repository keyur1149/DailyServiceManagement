...
const handleReRender = () => {  
 ...
 setReRender(!reRender); // state change will re-render parent      
  ...
  notdelivered.includes(e)?<Onedate key={index} month={month} year={year} color="blue"  value={e}/>:<Onedate key={index} month={month} year={year} color="red" onReRender={handleReRender} ischecked={true} value={e}/>
  )
  const now1=predates.map((e,index)=>
  notdelivered.includes(e)?<Onedate key={index} month={month} year={year} color="blue"  value={e}/> :<Onedate key={index} month={month} year={year} color="#08e908" value={e}/>)

  return(
    <div>
      <div style={{display: "inline-flex",
    flexWrap: "wrap",    justifyContent: "space-evenly"}}>{now1}{now}</div>
    </div>
  )
}
